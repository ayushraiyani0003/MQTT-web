...
function TopicTree({
    ...
    const [topicLastUpdated, setTopicLastUpdated] = useState({});
    const previousMessagesRef = useRef({});
    const timeoutRefsMap = useRef({});
    const commCheckIntervalRef = useRef(null);
    ...
    useEffect(() => {
        ...
        const newTopicLastUpdated = { ...topicLastUpdated };
        ...
        Object.keys(messages).forEach((topic) => {
            if (
                ...
                newRecentlyUpdated[topic] = true;
                newTopicLastUpdated[topic] = Date.now();
                hasChanges = true;
                ...
                if (timeoutRefsMap.current[topic]) {
                    clearTimeout(timeoutRefsMap.current[topic]);
                }

                // Set timeout to remove the highlight after 200 microseconds
                timeoutRefsMap.current[topic] = setTimeout(() => {
                    setRecentlyUpdated((prev) => {
                        const updated = { ...prev };
                        delete updated[topic];
                        return updated;
                    });
                }, 0.2); // 200 microseconds = 0.2 milliseconds
            }
        });
        ...
        if (hasChanges) {
            ...
            setTopicLastUpdated(newTopicLastUpdated);
        }

        // Store current messages for comparison in next update
        previousMessagesRef.current = JSON.parse(JSON.stringify(messages));

        // Setup comm status check interval when component mounts
        if (!commCheckIntervalRef.current) {
            commCheckIntervalRef.current = setInterval(() => {
                // This will force re-render to update comm status indicators
                setTopicLastUpdated((prev) => ({ ...prev }));
        ...
        return () => {
            // Clear all timeouts
            Object.values(timeoutRefsMap.current).forEach((timeoutId) => {
                clearTimeout(timeoutId);
            });

            // Clear comm status check interval
            if (commCheckIntervalRef.current) {
                clearInterval(commCheckIntervalRef.current);
                commCheckIntervalRef.current = null;
            }
        };
    }, [messages, recentlyUpdated, topicLastUpdated]);
    ...
    // Check if a topic has a status message that's more than 2 minutes old
    const hasStaleStatusTopic = (topicPath) => {
        // Check if this topic has a status subtopic
        const statusTopicPath = `${topicPath}/status`;
        const now = Date.now();
        const twoMinutesMs = 2 * 60 * 1000; // 2 minutes in milliseconds

        // If this topic has a status subtopic and it's been more than 2 minutes
        // since last update, we should show the "comm" indicator
        if (
            topicLastUpdated[statusTopicPath] &&
            now - topicLastUpdated[statusTopicPath] > twoMinutesMs
        ) {
            return true;
        }

        // Also check if status is part of the path and it's the direct topic
        if (
            topicPath.includes("/status") &&
            topicLastUpdated[topicPath] &&
            now - topicLastUpdated[topicPath] > twoMinutesMs
        ) {
            return true;
        }
    ...
    const renderTopicNode = (name, node, path = "", level = 0) => {
        ...
        const showCommIndicator = !isLeaf && hasStaleStatusTopic(displayPath);
        ...
        const nodeClasses = [
            ...
            isLeaf ? "leaf" : "branch",
            showCommIndicator ? "comm-warning" : "",
        ]
            .filter(Boolean)
            .join(" ");
