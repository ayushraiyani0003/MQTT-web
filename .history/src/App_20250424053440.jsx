...
function App() {
    ...
    const connectMqtt = (credentials) => {
        ...
        try {
            ...
            // Make sure port is a string and falls back to appropriate WebSocket ports
            const port =
                credentials.port || (credentials.useTLS ? "8084" : "8083");

            // Make sure path has a leading slash
            const path = credentials.path || "/mqtt";
            const formattedPath = path.startsWith("/") ? path : `/${path}`;

            // WebSocket URL format
            const url = `${wsProtocol}://${credentials.brokerUrl}:${port}${formattedPath}`;
