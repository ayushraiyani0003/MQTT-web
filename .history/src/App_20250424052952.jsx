...
function App() {
    ...
    const connectMqtt = (credentials) => {
        ...
        try {
            // For browsers, we must use WebSockets - standard MQTT won't work
            const protocol = credentials.useTLS ? "mqtt" : "mqtts";
            const port = credentials.port || (credentials.useTLS ? 8084 : 8083); // Common MQTT WebSocket ports

            // Direct WebSocket URL format
            const url = `${protocol}://${credentials.brokerUrl}:${port}${
                credentials.path || ""
            }`;
