...
function App() {
    ...
    const [splitPosition, setSplitPosition] = useState(300); // Adjusted default width
    ...
    const connectMqtt = (credentials) => {
        ...
        try {
            // Construct the broker URL - important fix here
            // For MQTT over WebSockets, we need to use the correct format and port
            let url;

            // Using a direct MQTT URL format
            if (credentials.useTLS) {
                // Secure WebSocket
                url = `mqtts://${credentials.brokerUrl}:${
                    credentials.port || 8883
                }`;
            } else {
                // Regular WebSocket
                url = `mqtt://${credentials.brokerUrl}:${
                    credentials.port || 1883
                }`;
            }
            ...
            const options = {
                ...
                // WebSocket options
                protocol: credentials.useTLS ? "wss" : "ws",
                // If needed, direct WebSocket paths can be specified
                path: credentials.path || "/mqtt",
            };

            // Create MQTT client with the enhanced options
            ...
            client.on("message", (topic, payload) => {
                ...
                addMessage(topic, content, format, timestamp);
            });
            ...
            setTimeout(() => {
                if (!client.connected) {
                    setConnectionError(
                        "Connection timeout - check broker settings"
                    );
                }
